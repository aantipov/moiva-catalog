const libs =  {
  name: 'misc',// Do not change this name. It's "reserved"
  items: [
    // <npm-package-name> or [<npm-package-name>, <seo-alias>]
    // Seo aliases are used to refer to npm packages in Google Search results. For example, for 'angular/core' Google will show results with a title "Angular: Stats and Trends - Moiva.io".
    // Aliases are important to help users find the right page
    // Important: when adding an alias, make sure it's unique - no other packages in this file should use it as their name
    '@nivo/core',
    'apisauce',
    'async',
    'axios',
    'bluebird',
    'browserify',
    'buefy',
    'chai',
    'commander',
    'commitizen',
    'classnames',
    'cheerio',
    'core-js',
    'debug',
    'fp-ts',
    'grunt',
    'gulp',
    'inquirer',
    'jasmine',
    'jquery',
    'karma',
    'less',
    'mkdirp',
    'mocha',
    'node-fetch',
    'passport',
    'prop-types',
    'protractor',
    'react-chartjs-2',
    'react-router',
    'react-vis',
    'sanctuary',
    'sass',
    'superagent',
    'universal-router',
    'uuid',
    'victory',
    'yargs',
    ['postcss', 'PostCSS'],
    ['create-react-app', 'Create React App'],
    'react-router-dom',
    'react-native',
    'electron',
    'animate.css',
    ['@fortawesome/fontawesome-free', 'Font Awesome'],
    'reveal.js',
    ['@ionic/core', 'Ionic'],
    'normalize.css',
    ['material-design-icons', 'Material Icons'],
    'yarn',
    ['serverless', 'Serverless Framework'],
    ['@babel/core', 'Babel'],
    'animejs',
    'impress.js',
    'ghost',
    'mermaid',
    'strapi',
    ['pixi.js', 'PixiJS'],
    ['fullpage.js', 'fullPage.js'],
    ['@atomist/hackathon-starter', 'Hackathon Starter'],
    'video.js',
    ['clipboard', 'clipboard.js'],
    ['faker', 'faker.js'],
    'phaser',
    'quill',
    ['ant-design-pro', 'Ant Design Pro'],
    ['@vue/cli', 'Vue CLI', 'vue'],
    ['slick-carousel', 'Slick'],
    'lerna',
    'swiper',
    'request',
    ['standard', 'JavaScript Standard Style'],
    'modernizr',
    ['whatwg-fetch', 'Fetch'],
    ['xlsx', 'SheetJS js-xlsx'],
    'marked',
    ['@angular/cli', 'Angular CLI', 'angular'],
    ['@uppy/core', 'Uppy'],
    'sequelize',
    ['webtorrent', 'WebTorrent'],
    'webtorrent-cli',
    ['lottie-web', 'Lottie for Web'],
    ['monaco-editor', 'Monaco Editor'],
    'tesseract.js',
    ['typeorm', 'TypeORM'],
    ['@tabler/core', 'Tabler'],
    ['ace-builds', 'Ace'],
    'react-beautiful-dnd',
    'mongoose',
    ['hammerjs', 'hammer.js'],
    ['codemirror', 'CodeMirror'],
    ['sortablejs', 'Sortable'],
    'lighthouse',
    ['@polymer/polymer', 'Polymer'],
    'polymer-cli',
    'husky',
    ['purecss', 'Pure.css'],
    'redux-saga',
    ['jspdf', 'jsPDF'],
    ['@angular/material', 'Material for Angular'],
    'react-virtualized',
    'react-window',
    'dragula',
    'intro.js',
    'normalizr',
    ['draft-js', 'Draft.js'],
    'nativescript',
    ['feather-icons', 'Feather'],
    'slate',
    'scrollreveal',
    'react-motion',
    ['@mostly-adequate/support', 'Guide to FP'],
    'react-spring',
    'autoprefixer',
    'flv.js',
    'plyr',
    'sharp',
    ['localforage', 'localForage'],
    'react-use',
    'popmotion',
    ['react-native-web', 'React Native for Web'],
    ['fabric', 'Fabric.js'],
    'highlight.js',
    ['howler', 'Howler.js'],
    ['vue-router', 'Vue Router'],
    'reselect',
    'vux',
    'pkg',
    ['@blueprintjs/core', 'Blueprint', 'react'],
    ['react-query', 'React Query'],
    ['validator', 'validator.js'],
    'flux',
    ['js-cookie', 'JavaScript Cookie'],
    'ccxt',
    ['velocity-animate', 'Velocity']
    ['angular-material', 'Material for AngularJS', 'angularjs'],
    'egg',
    ['@popperjs/core', 'Popper'],
    ['uikit', 'UIkit'],
    ['vant', null, 'vue'],
    ['@mojs/core', 'mojs'],
    'cleave.js',
    ['mint-ui', 'Mint UI'],
    ['file-saver', 'FileSaver.js'],
    ['streamsaver', 'StreamSaver.js'],
    'vite',
    'framework7',
    'trix',
    'chalk',
    ['react-admin', null, 'react'],
    ['coffeescript', 'CoffeeScript'],
    'react-loadable',
    'ink',
    ['dva', 'DvaJS'],
    'svgo',
    'jsdom',
    'aos',
    ['swr', 'SWR'],
    'ar.js',
    'masonry-layout',
    ['react-dnd', 'React DnD'],
    'ionicons',
    ['rxdb', 'RxDB'],
    ['roughjs', 'Rough.js'],
    ['medium-editor', 'MediumEditor'],
    'q',
    ['zepto', 'Zepto.js'],
    ['@shopify/draggable', 'Draggable'],
    'p5',
    'lazysizes',
    'handsontable',
    ['recompose', null, 'react'],
    'lowdb',
    'native-base',
    ['@tensorflow/tfjs', 'TensorFlow.js'],
    'xstate',
    ['@editorjs/editorjs', 'Editor.js'],
    'prepack',
    'n',
    'blessed-contrib',
    'blessed',
    'weex-vue-render',
    ['pouchdb', 'PouchDB'],
    ['@theia/core', 'Eclipse Theia'],
    'flatpickr',
    ['frappe-charts', 'Frappe Charts'],
    ['knex', 'knex.js'],
    'jsonwebtoken',
    ['vivus', 'vivus.js'],
    ['@fullcalendar/core', 'FullCalendar'],
    'nodemailer',
    ['react-table', 'React Table', 'react'],
    ['store', 'Store.js'],
    ['netlify-cms-app', 'Netlify CMS'],
    ['katex', 'KaTeX'],
    ['snapsvg', 'Snap.svg'],
    ['dotenv'],
    ['bit-bin', 'Bit'],
    'expo-cli',
    'expo',
    ['react-grid-layout', 'React-Grid-Layout', 'react'],
    ['headless-recorder', 'Headless Recorder'],
    'bootswatch',
    ['selectize', 'selectize.js'],
    'turbolinks',
    'gun',
    ['super-tiny-icons', 'Super Tiny Icons'],
    ['grapesjs', 'GrapesJS'],
    'sweetalert2',
    ['aframe', 'A-Frame'],
    ['@toast-ui/editor', 'TOAST UI Editor'],
    ['react-intl', 'React Intl'],
    ['@vx/vx', 'visx'],
    ['paper', 'Paper.js'],
    'driver.js',
    'twemoji',
    'n8n',
    'johnny-five',
    ['gpu.js', 'GPU.js'],
    ['react-native-navigation', 'React Native Navigation'],
    ['react-hot-loader', 'React Hot Loader'],
    ['gsap', 'GSAP'],
    'fuse.js',
    ['verbal-expressions', 'VerbalExpressions'],
    'brain.js',
    'omi',
    'react-three-fiber',
    ['shelljs', 'ShellJS'],
    ['smartcrop', 'smartcrop.js'],
    ['mjml', 'MJML'],
    'pell',
    ['node-red', 'Node-RED'],
    'co',
    ['nanoid', 'Nano ID'],
    ['browser-sync', 'Browsersync'],
    ['systemjs', 'SystemJS'],
    'pomelo',
    'verdaccio',
    ['matter-js', 'Matter.js'],
    ['redux-persist', 'Redux Persist'],
    'virtual-dom',
    'markdown-it',
    ['apollo-server', 'Apollo Server'],
    ['@assemblyscript/loader', 'AssemblyScript'],
    'jimp',
    ['jquery-ui', 'jQuery UI'],
    'raphael',
    ['supertest', 'SuperTest'],
    ['x-data-spreadsheet', 'x-spreadsheet'],
    ['mathjs', 'Math.js'],
    'headroom.js',
    'proton-native',
    ['dplayer', 'DPlayer'],
    ['tone', 'Tone.js'],
    'stylus',
    'crypto-js',
    'mousetrap',
    ['@antv/g2', 'G2'],
    ['@antv/g2plot','G2Plot'],
    ['@swc/core', 'swc'],
    'filepond',
    ['react-content-loader', null, 'react'],
    'list.js',
    ['@mdx-js/mdx', 'MDX'],
    'fx',
    ['@fluentui/react', 'Fluent UI Web/React'],
    'nock',
    'botkit',
    'electron-builder',
    'vue-resource',
    'bull',
    ['@primer/css', 'Primer CSS'],
    'web-vitals',
    ['@vueuse/core', 'VueUse'],
  ]
};

module.exports = libs;
